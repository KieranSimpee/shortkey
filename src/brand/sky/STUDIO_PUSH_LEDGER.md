...
|------|------|---------|--------|
| 2026-07-24 | Family Table | **Family Home Doorbell + Receipt Board v0.1** | Living Room command with `target_members` (`sky` · `senti` · `kura` · `agent-r` · `gor-gor` · `all`). Receipt rows per target at **SENT** (not auto RECEIVED). Member rooms: ack buttons 收到/睇緊/處理中/需要Gor Gor/Blocked/已提交 + optional note; room chat reply also → RECEIVED. Living Room Receipt Board: member · status · timestamp · last note. Storage **`shortkey-doorbell-receipts-v01`** (localStorage · **local prototype only** · no cross-device). Soft pearl/lavender · Internal Staging until Gor Gor Review. Gor Gor Chat Bridge stays separate. **Coming Soon untouched · no secrets.** Doc: [`FAMILY_HOME_DOORBELL_RECEIPT_v0_1.md`](./FAMILY_HOME_DOORBELL_RECEIPT_v0_1.md). Status: **Internal Staging · Gor Gor Review pending**. |
