...
|------|------|---------|--------|
| 2026-07-24 | Family Home | **v0.9.1 · Doorbell & Receipt Board** | Living Room command with `target_members` (`sky` · `senti` · `kura` · `agent-r` · `gor-gor` · `all`). Receipt rows per target at **SENT** (not auto RECEIVED). Member rooms: ack 收到/睇緊/處理中/需要Gor Gor/Blocked/已提交 + optional note; room chat reply also → RECEIVED. Living Room Receipt Board: member · status · timestamp · last note. Storage **`shortkey-family-doorbell-v091`** (migrates once from `shortkey-doorbell-receipts-v01`). **local prototype only · not shared across devices**. Soft pearl/lavender · Internal Staging until Gor Gor Review. Gor Gor Chat Bridge stays separate. **Coming Soon untouched · no secrets.** Doc: [`FAMILY_HOME_v0_9_1_DOORBELL_RECEIPT.md`](./FAMILY_HOME_v0_9_1_DOORBELL_RECEIPT.md). Status: **Internal Staging · Gor Gor Review pending**. |
