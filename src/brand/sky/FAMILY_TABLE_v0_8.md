...
| **`shortkey-family-doorbell-v092`** | Family Home **v0.9.3** Family Meeting UI — localStorage **fallback/demo only**; canonical shared store via `/api/family-doorbell/*` (Redis key v092 preserved; see [`FAMILY_HOME_v0_9_3_FAMILY_MEETING_UI.md`](./FAMILY_HOME_v0_9_3_FAMILY_MEETING_UI.md)); migrates once from v091 / v01 keys |
...
**Shared DB:** Family Table house panels = localStorage only. **Doorbell / Family Meeting v0.9.3** = shared API when store configured (file/Upstash/KV); otherwise local fallback.  
