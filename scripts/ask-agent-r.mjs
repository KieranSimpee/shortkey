/**
 * Ask Agent R (Base44 Superagent) — Secretary · customer care · records.
 * Secrets: KURA_API_KEY or BASE44_API_KEY from env / .env.local (never commit).
 * Same shared key as Kura · Gor Gor · Senti.
 *
 * Usage:
 *   npm run ask:agent-r -- "Log this decision for the archive"
 *   node scripts/ask-agent-r.mjs "your message"
 *
...
const DEFAULT_AGENT_ID = "6a449e8691d185359beef333";
...
const agentId = (process.env.AGENT_R_AGENT_ID || "").trim() || DEFAULT_AGENT_ID;
...
4. Optional: AGENT_R_AGENT_ID=${DEFAULT_AGENT_ID}
5. Re-run: npm run ask:agent-r -- "your question"
...
if (!message) {
  console.error('Usage: npm run ask:agent-r -- "your question for Agent R"');
  process.exit(1);
}
