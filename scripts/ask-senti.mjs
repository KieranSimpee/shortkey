/**
 * Ask Senti (Base44 Superagent) — sister · Creative Director · Document Execution.
 * Secrets: KURA_API_KEY or BASE44_API_KEY from env / .env.local (never commit).
 * Same shared key as Kura · Gor Gor · Agent R.
 *
 * Usage:
 *   npm run ask:senti -- "Draft the creative brief for this issue"
 *   node scripts/ask-senti.mjs "your message"
 *
...
const DEFAULT_AGENT_ID = "6a42029cc124d0206f027335";
const BASE_ROOT = "https://app.base44.com/api/agents";
...
const agentId = (process.env.SENTI_AGENT_ID || "").trim() || DEFAULT_AGENT_ID;
const BASE = `${BASE_ROOT}/${agentId}`;
...
if (!apiKey) {
  console.error(`
Missing API key for Senti.

1. Open any Base44 family agent Editor → Settings → Developer → API Key
2. Copy .env.example → .env.local (if needed)
3. Set:  KURA_API_KEY=your_key_here
   (accepted aliases: BASE44_API_KEY · BASE44_AGENT_API_KEY — same shared value)
4. Optional: SENTI_AGENT_ID=${DEFAULT_AGENT_ID}
5. Re-run: npm run ask:senti -- "your question"
...
if (!message) {
  console.error('Usage: npm run ask:senti -- "your question for Senti"');
  process.exit(1);
}
